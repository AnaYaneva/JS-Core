   ...
   async function login() {
    ...
    async function loadAds() {
        ...
        for(let ad of data){
            ...
            //alert(ad._acl.creator+" + "+localStorage.getItem('id'));
            let detailBtn = $('<button>&#9906;</button>').click(()=> displayAdvert(ad));
            detailBtn.addClass('ad-control');
            detailBtn.appendTo(title);
            ...
            html.append(`<div>By ${ad.publisher} \| Price: ${Number(ad.price).toFixed(2)} lv</div>`);
    ...
    async function createAd() {
        ...
        let price=form.find('input[name="price"]').val();
        let imageUrl=form.find('input[name="image"]').val();
        let date=(new Date()).toString('yyyu-MM-dd');
    ...
    function displayAdvert(ad) {
        let html=$('<div>');
        html.addClass('ad-view');
        let title=$(`<div class="ad-title">${ad.title}</div>`);

        html.append(title);
        html.append(`<div><img src="${ad.imageUrl}"></div>`).append('<br>');;
        html.append(`<div>By ${ad.publisher}</div>`).append('<br>');;
        html.append(`<div>Price: ${Number(ad.price).toFixed(2)} lv</div>`).append('<br>');;
        html.append(`<div>Description: ${ad.description}</div>`).append('<br>');
        html.append(`<div>Date published: ${ad.date.toString('yyyy-MM-dd')}</div>`);
